...
# Shorten all survival times to < 5 years
study_population <-
  study_population %>%
  mutate(
    c_timetodeath_years_short = case_when(
      c_timetodeath_years <= 5 ~ c_timetodeath_years,
      T ~ 5
    ),
    c_dead_short = case_when(
      c_timetodeath_years <= 5 ~ c_dead,
      T ~ 0
    )
  ) 
...
rel_mod_all <-
  study_population %>%
  stpm2(Surv(c_timetodeath_years_short, c_dead_short) ~ nsx(c_age, df = 2) + nsx(d_indexyear, df = 2) + rt_sex,
        data = .,
        tvc = list(
          c_age = 2,
          d_indexyear = 2,
...
rel_mod_normal_ef <-
  study_population %>%
  filter(normal_ef == ">=50%") %>%
  stpm2(Surv(c_timetodeath_years_short, c_dead_short) ~ nsx(c_age, df = 2) + rt_sex + nsx(d_indexyear, df = 2),
        data = .,
        tvc = list(
          c_age = 2,
          d_indexyear = 2,
...
newdata <- expand_grid(
  c_timetodeath_years_short = 0:300 / 10,
  c_age = c(50, 60, 70, 80),
...
lole_measures %>%
  saveRDS(., file = "output/LOLE_MEASURES_UNADJ_SENS.rds")
